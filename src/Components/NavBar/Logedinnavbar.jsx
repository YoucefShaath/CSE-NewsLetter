...
export default function Logedinnavbar() {
  ...
  const departments = [
    ...
    { name: "Multimedia", href: "/department/multimedia", icon: <FaCamera /> },
    {
      name: "Following",
      href: "/department/following",
      icon: <FaUserFriends />,
    },
  ];

  const canCreatePost = user?.role !== "Member";
          ...
          <div className="flex items-center gap-4">
            {canCreatePost && (
              <button
                ...
                className={`transition-colors flex items-center justify-center w-8 h-8 rounded-full border ${
                  isDark
              ...
              <div className="flex justify-start md:justify-center gap-4 flex-nowrap overflow-x-auto max-w-full mx-auto pb-2">
                {departments.map((dept) => (
                  <Link
                    key={dept.name}
                    href={dept.href}
                    className={`group relative px-4 md:px-6 py-3 whitespace-nowrap rounded-2xl text-sm font-medium transition-all duration-200 flex items-center gap-2 shrink-0 ${
                      isDark
